...
class TestDataLoader(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.mock_generation_data = pd.DataFrame({
            'DATE_TIME': ['2023-05-15 06:00:00', '2023-05-15 06:15:00', '2023-05-15 06:30:00'],
            'PLANT_ID': [1, 1, 1],
    ...
    def test_get_dataset_summary(self) -> None:
        ...
        self.assertEqual(summary['primary_target'], 'AC_POWER (solar power generation in kW)')
    
    @patch('pandas.read_csv')
    @patch('pathlib.Path.exists')
    def test_load_generation_data_success(self, mock_exists: MagicMock, mock_read_csv: MagicMock) -> None:
    ...
    @patch('pandas.read_csv')
    @patch('pathlib.Path.exists')
    def test_load_weather_data_success(self, mock_exists: MagicMock, mock_read_csv: MagicMock) -> None:
    ...
    @patch('pandas.read_csv')
    @patch('pathlib.Path.exists')
    def test_load_solar_data_success(self, mock_exists: MagicMock, mock_read_csv: MagicMock) -> None:
    ...
    @patch('pandas.read_csv')
    @patch('pathlib.Path.exists')
    def test_load_generation_data_missing_columns(self, mock_exists: MagicMock, mock_read_csv: MagicMock) -> None:
...
class TestDataLoaderIntegration(unittest.TestCase):
    ...
    @unittest.skipUnless(
        Path("data/raw/Plant_1_Generation_Data.csv").exists() and 
        Path("data/raw/Plant_1_Weather_Sensor_Data.csv").exists(),
    ...
    def test_load_real_dataset(self) -> None:
        ...
        try:
            ...
            print(f"Generation records: {validation_results['row_counts']['generation']:,}")
            print(f"Weather records: {validation_results['row_counts']['weather']:,}")
            
            if validation_results['data_quality_issues']:
                print(f"⚠️  Issues found: {len(validation_results['data_quality_issues'])}")
