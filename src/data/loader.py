...
def load_generation_data(data_path: Path = Path("data/raw")) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f"Loaded generation data: {df.shape[0]} rows, {df.shape[1]} columns")
...
def load_solar_data(data_path: Path = Path("data/raw")) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
...
def validate_data_integrity(
    ...
    if generation_df['PLANT_ID'].nunique() != 1 or generation_df['PLANT_ID'].iloc[0] != 1:
    ...
    if (weather_df['AMBIENT_TEMPERATURE'] < -50).any() or (weather_df['AMBIENT_TEMPERATURE'] > 60).any():
...
if __name__ == "__main__":
    ...
    Run this script to validate data loading after dataset download.
    """
    try:
        print("Loading solar power generation dataset...")
        
        # Load data
        generation_df, weather_df = load_solar_data()
        
        # Validate data
        validation_results = validate_data_integrity(generation_df, weather_df)
        
        # Print summary
        print("\n=== Dataset Summary ===")
        print(f"Generation data: {validation_results['row_counts']['generation']} rows")
        print(f"Weather data: {validation_results['row_counts']['weather']} rows")
        
        print(f"\nGeneration date range: {validation_results['date_ranges']['generation']['start']} to {validation_results['date_ranges']['generation']['end']}")
        print(f"Weather date range: {validation_results['date_ranges']['weather']['start']} to {validation_results['date_ranges']['weather']['end']}")
        
        if validation_results['data_quality_issues']:
            print(f"\n⚠️  Data quality issues found: {len(validation_results['data_quality_issues'])}")
        ...
        print("\nMake sure you have downloaded the dataset files to data/raw/:")
        print("  - Plant_1_Generation_Data.csv")
        print("  - Plant_1_Weather_Sensor_Data.csv")
    except Exception as e:
